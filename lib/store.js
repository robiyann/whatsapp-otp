...
class Store {
  ...
  replaceUserAccess(userId, numberIds) {
    const uniqueNumberIds = [...new Set((Array.isArray(numberIds) ? numberIds : []).filter(Boolean))];
    const deleteStmt = this.db.prepare('DELETE FROM access_entries WHERE user_id = ?');
    const insertStmt = this.db.prepare(`
      INSERT INTO access_entries
      (id, user_id, number_id, created_at)
      VALUES (?, ?, ?, ?)
    ...
    try {
      ...
      uniqueNumberIds.forEach(numberId => {
        insertStmt.run(this.generateId('acc'), userId, numberId, nowIso());
      });
      this.db.exec('COMMIT');
    } catch (error) {
